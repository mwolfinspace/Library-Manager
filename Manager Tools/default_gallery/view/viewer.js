...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const DISABLED_BINDING = "__disabled__";
  ...
  function readSessionStoryPassword(storyKey) {
    ...
    const store = loadPasswordStore(
      sessionStorage,
      PROTECTED_SESSION_PASSWORDS_KEY,
    );
  ...
  function clearStoredStoryPasswords(
    ...
    if (clearLocal) {
      const localStore = loadPasswordStore(
        localStorage,
        PROTECTED_LOCAL_PASSWORDS_KEY,
      );
    ...
    if (clearSession) {
      const sessionStore = loadPasswordStore(
        sessionStorage,
        PROTECTED_SESSION_PASSWORDS_KEY,
      );
  ...
  function consumeUnlockTicketPassword(storyKey) {
    ...
    const params = new URLSearchParams(window.location.search);
    const token = String(
      params.get(PROTECTED_UNLOCK_TICKET_PARAM) || "",
    ).trim();
    if (!token) {
      return "";
    }
  ...
  function runViewerIdleTask(task) {
    if (
  ...
  function preloadMediaItem(mediaItem) {
    if (!mediaItem || !mediaItem.src || mediaPreloadCache.has(mediaItem.src)) {
      return;
    }

    mediaPreloadCache.set(mediaItem.src, "pending");

    if (mediaItem.type === "video") {
      const video = document.createElement("video");
      const finish = (state) => {
        mediaPreloadCache.set(mediaItem.src, state);
      };
    ...
    img.onload = async () => {
      if (typeof img.decode === "function") {
        try {
          ...
          // The browser may still keep the loaded image in cache.
        }
      }
      mediaPreloadCache.set(mediaItem.src, "ready");
    };
    img.onerror = () => {
      mediaPreloadCache.set(mediaItem.src, "error");
    };
  ...
  function scheduleAdjacentMediaPreload() {
    ...
    const candidates =
      nextIndex === prevIndex
        ? [photos[nextIndex]]
        : [photos[nextIndex], photos[prevIndex]];

    runViewerIdleTask(() => {
      candidates.forEach(preloadMediaItem);
    });
  }
  ...
  function applySettings() {
    ...
    const fontObj =
      window.FONTS?.find(
        (f) =>
          f.name === settings.fontFamily || f.family === settings.fontFamily,
      ) || {};
    const fontFamily =
      fontObj.family ||
      (settings.customFont ? `'${settings.customFont}'` : null);
    ...
    if (els.fontFamily) {
      ...
      const fontByName = window.FONTS?.find(
        (f) => f.name === settings.fontFamily,
      );
      const fontByFamily = window.FONTS?.find(
    ...
    if (scrollStepValue)
      scrollStepValue.textContent = `${settings.scrollStep} px`;
    ...
    if (els.storyContent) {
      els.storyContent.style.scrollBehavior = settings.smoothScroll
        ? "smooth"
        : "auto";
    }
  ...
  function loadSettingsFromStorage() {
    try {
      if (settings.rememberViewAll) {
        const globalRaw = localStorage.getItem("viewerSettings:global");
        if (globalRaw) {
  ...
  function hydrateSettingsFromStorage() {
    ...
    settings.customBindings = Object.entries(
      settings.customBindings || {},
    ).reduce((acc, [action, key]) => {
      const normalized = normalizeKey(key);
      if (normalized || normalized === DISABLED_BINDING) {
  ...
  function saveSettings() {
    ...
    if (settings.rememberViewAll) {
      localStorage.setItem("viewerSettings:global", JSON.stringify(settings));
    } else {
  ...
  async function loadCatalog() {
    ...
    try {
      ...
      const cacheMap = new Map(catalogCache.map((item) => [item.id, item]));
  ...
  async function loadStoryMarkdown(path, entry, storyPassword = "") {
    ...
    if (entry && entry.storyText) {
      ...
      entry &&
      !entry.storyText &&
      Array.isArray(window.REPORT_CATALOG)
    ) {
    ...
    if (!rawStoryText) {
      ...
      rawStoryText = await fetchTextWithFallback(
        resolved,
        "Story file missing.",
      );
    }
  ...
  function updateBlackoutResumeText() {
    ...
    if (resumeKeyEl) {
      const profile =
        KEY_PROFILES[settings.keyboardMode] || KEY_PROFILES.default;
      const custom = getCustomBinding("blackout");
      const blackoutKey =
        custom === DISABLED_BINDING
          ? ""
          : custom || (profile.blackout && profile.blackout[0]) || "space";
      const keyDisplay = prettyKeyLabel(blackoutKey);
      resumeKeyEl.textContent = keyDisplay
        ? keyDisplay.toLowerCase() === "spacebar"
          ? "spacebar"
          : keyDisplay
        : "—";
    }
  ...
  function initPanelDragging() {
    ...
    document.addEventListener("mousemove", (e) => {
      ...
      els.settingsPanel.style.left =
        Math.max(
          0,
          Math.min(newLeft, window.innerWidth - els.settingsPanel.offsetWidth),
        ) + "px";
      els.settingsPanel.style.top =
        Math.max(
          0,
          Math.min(newTop, window.innerHeight - els.settingsPanel.offsetHeight),
        ) + "px";
    });
  ...
  function isModifierKey(key) {
    const normalized = normalizeKeyPart(key);
    return (
      normalized === "ctrl" ||
      normalized === "shift" ||
      normalized === "alt" ||
  ...
  function normalizeKeyCombo(value) {
    ...
    if (raw === DISABLED_BINDING) {
      return DISABLED_BINDING;
    }
    if (raw === "+") {
      return "plus";
    }
  ...
  function normalizeKey(key) {
    ...
    if (raw.toLowerCase() === DISABLED_BINDING) {
      return DISABLED_BINDING;
    }
    if (raw === "+") {
      return "plus";
  ...
  function buildKeyComboFromEvent(event) {
    ...
    const baseKey = normalizeKeyPart(event.key);
    if (!baseKey || isModifierKey(baseKey)) {
      return "";
    }
    const includeShift =
      event.shiftKey &&
  ...
  function getCustomBinding(action) {
    const value =
      settings.customBindings && settings.customBindings[action]
    ...
    if (normalized === DISABLED_BINDING) {
      return DISABLED_BINDING;
    }
    return normalized;
  }

  function keyMatches(list, keyCombo, action) {
    const normalized = normalizeKey(keyCombo);
    const custom = action ? getCustomBinding(action) : null;
    if (custom) {
      if (custom === DISABLED_BINDING) {
        return false;
      }
  ...
  function getKeybindDefinitions() {
    ...
    return [
      ...
      // Story Navigation
      {
        ...
        keys: ["ctrl+arrowleft", "pageup"],
        category: "story",
      },
      {
        action: "nextStory",
        label: "Next story",
        keys: ["ctrl+arrowright", "pagedown"],
        category: "story",
      },
      // Zoom Controls
      {
      ...
      // Interface Actions
      {
        ...
        keys: profile.favorite,
        category: "ui",
      },
      {
        action: "bookmark",
        label: "Pin this story",
        keys: profile.bookmark,
        category: "ui",
      },
      {
        action: "toggleTheme",
      ...
      {
        action: "blackout",
        label: "Blackout screen mode",
        keys: profile.blackout,
        category: "ui",
      },
      {
        action: "goToGallery",
        label: "Back to gallery",
        keys: profile.goToGallery,
        category: "ui",
      },
    ];
  }

  function getPrimaryBindingForAction(action) {
    const bindings = getKeybindDefinitions();
    const match = bindings.find((binding) => binding.action === action);
    if (!match) {
      return "";
    }
    const custom = getCustomBinding(action);
    ...
    const fallback = match.keys && match.keys.length > 0 ? match.keys[0] : "";
    return normalizeKey(fallback);
  }

  function disableDuplicateBindings(action, keyCombo) {
    const normalized = normalizeKey(keyCombo);
    if (!normalized) {
      return;
    }
    ...
    bindings.forEach((binding) => {
      ...
      const current = getPrimaryBindingForAction(binding.action);
      if (current && current === normalized) {
        if (!settings.customBindings) {
          settings.customBindings = {};
        }
        settings.customBindings[binding.action] = DISABLED_BINDING;
      }
    });
  }
  ...
  function renderKeybindList() {
    ...
    const allBindings = getKeybindDefinitions();
  ...
  function handleKeydown(event) {
    ...
    if (els.settingsPanel && !els.settingsPanel.hidden) {
      ...
      if (bindingTarget) {
        ...
        const pressed = buildKeyComboFromEvent(event);
        if (!pressed) {
          return;
        }
        disableDuplicateBindings(bindingTarget, pressed);
        if (!settings.customBindings) {
          settings.customBindings = {};
        }
    ...
    const keyCombo = buildKeyComboFromEvent(event);

    if (keyMatches(profile.toggleSettings, keyCombo, "toggleSettings")) {
      event.preventDefault();
      showSettings();
      return;
    }

    if (keyMatches(profile.blackout, keyCombo, "blackout")) {
      event.preventDefault();
      toggleBlackout();
      return;
    }

    if (keyMatches(profile.goToGallery, keyCombo, "goToGallery")) {
      event.preventDefault();
      window.location.href = appendViewerSource(resolveBackBaseUrl());
      return;
    }

    if (keyMatches(profile.favorite, keyCombo, "favorite")) {
      event.preventDefault();
      toggleFavorite();
      return;
    }

    if (keyMatches(profile.bookmark, keyCombo, "bookmark")) {
      event.preventDefault();
      toggleBookmark();
      return;
    }

    if (keyMatches(profile.toggleTheme, keyCombo, "toggleTheme")) {
      event.preventDefault();
    ...
    if (keyMatches(profile.prev, keyCombo, "prev")) {
      event.preventDefault();
      if (photos.length > 0) {
        currentPhotoIndex =
          (currentPhotoIndex - 1 + photos.length) % photos.length;
    ...
    if (keyMatches(profile.next, keyCombo, "next")) {
      event.preventDefault();
      if (photos.length > 0) {
        currentPhotoIndex = (currentPhotoIndex + 1) % photos.length;
        updatePhoto();
    ...
    if (!event.ctrlKey && !event.shiftKey) {
      if (settings.panKeys) {
        ...
        if (keyMatches(["w", "arrowup"], keyCombo, "panUp")) {
          translateY -= panStep;
          panHandled = true;
        } else if (keyMatches(["s", "arrowdown"], keyCombo, "panDown")) {
          translateY += panStep;
          panHandled = true;
        } else if (keyMatches(["a", "arrowleft"], keyCombo, "panLeft")) {
          translateX -= panStep;
          panHandled = true;
        } else if (keyMatches(["d", "arrowright"], keyCombo, "panRight")) {
          translateX += panStep;
          panHandled = true;
        }
    ...
    if (keyMatches(profile.zoomIn, keyCombo, "zoomIn")) {
      event.preventDefault();
    ...
    if (keyMatches(profile.zoomOut, keyCombo, "zoomOut")) {
      event.preventDefault();
    ...
    if (keyMatches(profile.fit, keyCombo, "fit")) {
      event.preventDefault();
      resetView();
      saveZoomState();
      return;
    ...
    if (keyMatches(profile.fontUp, keyCombo, "fontUp")) {
      event.preventDefault();
    ...
    if (keyMatches(profile.fontDown, keyCombo, "fontDown")) {
      event.preventDefault();
    ...
    if (keyMatches(profile.lineUp, keyCombo, "lineUp")) {
      event.preventDefault();
    ...
    if (keyMatches(profile.lineDown, keyCombo, "lineDown")) {
      event.preventDefault();
  ...
  if (els.rememberViewAll) {
    els.rememberViewAll.addEventListener("change", () => {
      const wasGlobal = settings.rememberViewAll;
      const willBeGlobal = els.rememberViewAll.checked;
      settings.rememberViewAll = willBeGlobal;
      ...
      if (!wasGlobal && willBeGlobal) {
        localStorage.setItem("viewerSettings:global", JSON.stringify(settings));
      } else if (wasGlobal && !willBeGlobal) {
        localStorage.setItem("viewerSettings", JSON.stringify(settings));
      }
    });
  }
    ...
    try {
      ...
      if (isEntryProtected(storyEntry)) {
        ...
        try {
          ...
          showLocked(
            "Unable to unlock. Return to homepage and enter password again.",
          );
          return;
        }
      } else {
  ...
  const resetConfig = {
    "pin-favorite": {
      ...
      message:
        "This will remove all pinned and favorited stories. Recent timestamps will remain intact. Are you sure?",
      isDanger: false,
    },
    pin: {
      title: "Reset Pin Only",
      message:
        "This will remove all pinned stories. Favorites and timestamps will be preserved. Are you sure?",
      isDanger: false,
    },
    favorite: {
      title: "Reset Favorite Only",
      message:
        "This will remove all favorited stories. Pins and timestamps will be preserved. Are you sure?",
      isDanger: false,
    },
    reading: {
      title: "Reset Reading Status",
      message:
        "This will remove ALL pins, favorites, and timestamps. All posts will become new/unread. This action cannot be undone.",
      isDanger: true,
    },
    settings: {
      title: "Reset All Settings",
      message:
        "This will reset all colors, fonts, sizes, and per-report settings to default. This action cannot be undone.",
      isDanger: true,
    },
  };
