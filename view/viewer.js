...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateCursorGlow(xRatio, yRatio) {
        ...
        els.codeField.style.setProperty('--cursor-x', `${(xRatio * 100).toFixed(2)}%`);
        els.codeField.style.setProperty('--cursor-y', `${(yRatio * 100).toFixed(2)}%`);
    }
    ...
    function refreshHudMarks() {
        ...
        els.hudMarks.forEach(mark => {
            ...
            mark.textContent = show
                ...
                : '';
            mark.style.opacity = show ? (0.45 + Math.random() * 0.45).toFixed(2) : '0';
    ...
    function applySettings() {
        ...
        if (els.rememberViewAll) els.rememberViewAll.checked = settings.rememberViewAll;
    ...
    function setupBackLink() {
        ...
        els.backLink.href = safe.startsWith('http') || safe.startsWith('/') ? safe : `../${safe}`;
    ...
    async function loadCatalog() {
        try {
            ...
            const catalogCache = Array.isArray(window.REPORT_CATALOG) ? window.REPORT_CATALOG : [];
            if (catalogCache.length === 0) {
                return list;
            }
            const cacheMap = new Map(catalogCache.map(item => [item.id, item.storyText]));
    ...
    function applyStoryEntry(entry) {
        storyEntry = entry;
        const title = entry.title || (entry.reportNumber ? `The Report #${entry.reportNumber}` : 'Story');
        ...
        photos = Array.isArray(entry.images) ? sortPhotos(entry.images).map(resolvePath) : [];
        failedLoads = 0;
    ...
    function saveZoomState() {
        ...
        localStorage.setItem(getStorageKey('zoomLevel', { global: true }), zoomLevel);
        localStorage.setItem(getStorageKey('translateX', { global: true }), translateX);
        localStorage.setItem(getStorageKey('translateY', { global: true }), translateY);
    }

    function restoreZoomState() {
        zoomLevel = parseFloat(localStorage.getItem(getStorageKey('zoomLevel', { global: true }))) || 1;
        translateX = parseFloat(localStorage.getItem(getStorageKey('translateX', { global: true }))) || 0;
        translateY = parseFloat(localStorage.getItem(getStorageKey('translateY', { global: true }))) || 0;
        updateTransform();
    }

    function saveScrollPosition() {
        localStorage.setItem(getStorageKey('scrollPosition'), els.storyContent.scrollTop);
    }

    function restoreScrollPosition() {
        const scrollPosition = localStorage.getItem(getStorageKey('scrollPosition'));
        if (scrollPosition) {
    ...
    function getCustomBinding(action) {
        return settings.customBindings && settings.customBindings[action] ? settings.customBindings[action] : null;
    }
    ...
    function formatBinding(action, fallbackKeys) {
        ...
        const fallback = fallbackKeys && fallbackKeys.length > 0 ? fallbackKeys[0] : '';
    ...
    function handleKeydown(event) {
        ...
        if (event.shiftKey && (key === 'w' || key === 's' || key === 'arrowup' || key === 'arrowdown')) {
            event.preventDefault();
            const delta = (key === 'w' || key === 'arrowup') ? -settings.scrollStep : settings.scrollStep;
            els.storyContent.scrollBy(0, delta);
            saveScrollPosition();
            return;
        }

        if (event.shiftKey && (key === 'a' || key === 'd' || key === 'arrowleft' || key === 'arrowright')) {
            event.preventDefault();
            if (photos.length === 0) {
                return;
            }
        ...
        if (settings.panKeys && ['w', 'a', 's', 'd', 'arrowup', 'arrowdown', 'arrowleft', 'arrowright'].includes(key)) {
            event.preventDefault();
            const panStep = 24;
        ...
        if (keyMatches(profile.lineUp, key, 'lineUp')) {
            event.preventDefault();
            settings.lineSpacing = Math.min(2.2, +(settings.lineSpacing + 0.1).toFixed(1));
            applySettings();
            saveSettings();
            return;
        }
        ...
        if (keyMatches(profile.lineDown, key, 'lineDown')) {
            event.preventDefault();
            settings.lineSpacing = Math.max(1.2, +(settings.lineSpacing - 0.1).toFixed(1));
            applySettings();
            saveSettings();
        }
    }
    ...
    els.photo.addEventListener('mouseleave', () => {
        isDragging = false;
        els.photo.classList.remove('dragging');
    });

    els.photo.addEventListener('mouseup', () => {
        isDragging = false;
        els.photo.classList.remove('dragging');
        saveZoomState();
    });

    els.photo.addEventListener('mousemove', event => {
    ...
    els.photo.addEventListener('wheel', event => {
        event.preventDefault();
